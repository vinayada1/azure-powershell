...
namespace Microsoft.Azure.Commands.Resources.Models.Authorization
{
	public class AuthorizationClient
	{
		...
		public List<PSRoleDefinition> FilterRoleDefinitions(string name, string scope, bool scopeAndBelow = false)
		{
			...
			ODataQuery<RoleDefinitionFilter> parameters = new ODataQuery<RoleDefinitionFilter>(f => f.RoleName == name);
		...
		{
			List<PSRoleDefinition> result = new List<PSRoleDefinition>();
			ODataQuery<RoleDefinitionFilter> parameters = new ODataQuery<RoleDefinitionFilter>(f => f.AtScopeAndBelow());
		...
		{
			List<PSRoleDefinition> result = new List<PSRoleDefinition>();
			ODataQuery<RoleDefinitionFilter> parameters;
			if (scopeAndBelow)
			{
				parameters = new ODataQuery<RoleDefinitionFilter>(f => f.AtScopeAndBelow());
			}
			else
			{
				parameters = new ODataQuery<RoleDefinitionFilter>();
			}
		...
		public PSRoleAssignment CreateRoleAssignment(FilterRoleAssignmentsOptions parameters)
		{
			...
			string roleDefinitionId = !string.IsNullOrEmpty(parameters.RoleDefinitionName)
				? AuthorizationHelper.ConstructFullyQualifiedRoleDefinitionIdFromScopeAndIdAsGuid(parameters.Scope, GetSingleRoleDefinitionByName(parameters.RoleDefinitionName, parameters.Scope).Id.ToString())
		...
		public PSRoleDefinition RemoveRoleDefinitionById(string roleDefinitionId, string scope)
		{
			...
			{
				return AuthorizationManagementClient.RoleDefinitions.Delete(scope, roleDefinitionId).ToPSRoleDefinition();
			}
			else
			{
		...
		public PSRoleDefinition RemoveRoleDefinitionByName(string roleDefinitionName, string scope)
		{
			...
			return AuthorizationManagementClient.RoleDefinitions.Delete(scope, roleDefinition.Id.ToString()).ToPSRoleDefinition();
		}

		public PSRoleDefinition RemoveRoleDefinition(FilterRoleDefinitionOptions options)
		{
			if (options.RoleDefinitionId.ToString() != string.Empty)
			{
				return this.RemoveRoleDefinitionById(options.RoleDefinitionId.ToString(), options.Scope);
			}
		...
		public PSRoleDefinition UpdateRoleDefinition(PSRoleDefinition roleDefinition)
		{
			...
			PSRoleDefinition fetchedRoleDefinition = this.GetRoleDefinition(roleDefinition.Id.ToString(), roleDefinition.AssignableScopes.First());
			...
			return this.CreateOrUpdateRoleDefinition(roleDefinition.Id.ToString(), roleDefinition);
		}
		...
		private PSRoleDefinition CreateOrUpdateRoleDefinition(string roleDefinitionId, PSRoleDefinition roleDefinition)
		{
			...
			try
			{
				roleDef = AuthorizationManagementClient.RoleDefinitions.CreateOrUpdate(roleDefinition.AssignableScopes.First(), roleDefinitionId, parameters).ToPSRoleDefinition();
			}
